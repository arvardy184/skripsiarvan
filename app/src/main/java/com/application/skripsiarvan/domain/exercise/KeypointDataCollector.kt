...
class KeypointDataCollector(private val context: Context) {
  ...
  fun startCollecting(label: String) {
    currentLabel = label
    isCollecting = true
    Log.d(TAG, "▶ Started collecting for label: $label")
  }

  /** Stop collecting data. */
  fun stopCollecting() {
    isCollecting = false
    Log.d(TAG, "⏹ Stopped collecting. Total samples: ${dataQueue.size}")
  }
  ...
  private fun exportWithMediaStore(fileName: String): String? {
    ...
    Log.d(TAG, "✅ Exported ${dataQueue.size} samples to $fileName")
    return uri.toString()
  }
  ...
  private fun exportToDownloads(fileName: String): String? {
    ...
    Log.d(TAG, "✅ Exported ${dataQueue.size} samples to ${file.absolutePath}")
