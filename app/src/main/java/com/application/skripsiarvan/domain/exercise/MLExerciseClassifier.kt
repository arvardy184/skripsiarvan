 ...
 *
 * Model Architecture:
 * - Input: 34 features (17 keypoints × [x, y], normalized 0-1)
 ...
 *
 * Rep counting logic: Uses phase transitions: STANDING → GOING_DOWN → AT_BOTTOM → GOING_UP →
 * STANDING = 1 rep
 */
class MLExerciseClassifier(context: Context) : ExerciseDetector {
  ...
  private fun loadModel(context: Context) {
    try {
      ...
      Log.d(TAG, "✅ Model loaded: input=$inputShape, output=$outputShape")
    } catch (e: Exception) {
   ...
   *
   * Valid rep sequence: STANDING → GOING_DOWN → AT_BOTTOM → GOING_UP → STANDING All phases must
    ...
    when (phase) {
      PHASE_STANDING -> {
        if (hasGoneUp && hasBeenAtBottom && hasGoneDown && hasBeenStanding) {
          ...
          Log.d(
            TAG,
            "✅ ML REP #$repetitionCount (conf: ${String.format("%.2f", confidence)})"
          )
          // Reset tracking
          resetPhaseTracking()
